...
int main(int argc, char ** argv) {
  ...
  // *init_r->zline = 2.0 - true_soln; %it don't like this
  *init_r->zline = 2.0-x(kk)*x(kk);
  ...
  // Boundary #1: x(0) = -1
 (*init_r->zline)(0,0,0) = dbc*(-1.0)*(-1.0) + // Dirichlet
                           -nbc*(2)*(-1.0); // Neumann part
  // Boundary #2: x(end) = 1
 (*init_r->zline)(0,0,N-1) = dbc*(1.0)*(1.0)  + // Dirichlet
                           nbc*(2)*(1.0); // Neumann part
