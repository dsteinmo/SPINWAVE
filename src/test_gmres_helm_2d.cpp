...
int main(int argc, char ** argv) {
   ...
   // set variable coefficients (ones for now)
   cx = 0*kk;   //coef of Dx
   cxx = 0*kk+1;  //coef of Dxx
   cy = 0*kk;   //coef of Dy
   cyy = 0*kk+1;  //coef of Dyy
   ...
   u_exact = cos(M_PI*y(kk))*cos(x(ii));

   write_array(u_exact,"u_exact");
   write_reader(u_exact,"u_exact");

   // set right-hand side as Helmholtzian of exact solution
   rhs = -cos(M_PI*y(kk))*cos(x(ii))*(2+M_PI*M_PI);
