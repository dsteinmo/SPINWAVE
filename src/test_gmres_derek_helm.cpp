...
int main(int argc, char ** argv) {
  ...
  a.set_bc(1.0,dbc,dbc,nbc,nbc); // set boundary conditions and helmholtz
                                 // first arg = 1.0 gives '-z' helm term. (i think)
  ...
  *init_r->zline = (M_PI*M_PI+1)*(-cos(M_PI*x(kk)));
  ...
  // Boundary #1: x(0) = -1
 (*init_r->zline)(0,0,0) = dbc*cos(M_PI*(-1.0)) + // Dirichlet
                           +nbc*M_PI*sin(M_PI*(-1.0)); // Neumann part
  // Boundary #2: x(end) = 1
 (*init_r->zline)(0,0,N-1) = dbc*cos(M_PI*(1.0)) + // Dirichlet
                           -nbc*M_PI*sin(M_PI*(1.0)); // Neumann part
